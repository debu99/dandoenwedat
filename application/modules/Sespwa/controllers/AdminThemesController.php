 ...
 * @version    $Id: AdminThemesController.php  2018-11-24 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sespwa_AdminThemesController extends Core_Controller_Action_Admin
{
    public function indexAction()
    {

        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespwa_admin_main', array(), 'sespwa_admin_main_managethemes');

        // Get themes
        $themes      = $this->view->themes      = Engine_Api::_()->getDbtable('themes', 'sespwa')->fetchAll();
        ...
        $reload_themes = false;
        foreach (glob(APPLICATION_PATH . '/application/themes/sespwa/*', GLOB_ONLYDIR) as $dir) {
        ...
        foreach ($themes as $theme) {
            if (!is_dir(APPLICATION_PATH . '/application/themes/sespwa/' . $theme->name)) {
                $theme->delete();
                $reload_themes = true;
            }
        }
        ...
        foreach ($themes as $key => $theme) {
            // Get theme manifest
            $themePath = "application/themes/sespwa/{$theme->name}";
        ...
        $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sespwa/'.$activeTheme->name.'/'.$activeFileName);
    }
    ...
    public function saveAction()
    {
        ...
        // Check file
        $basePath     = APPLICATION_PATH . '/application/themes/sespwa/' . $theme->name;
    ...
    {
        $themes = Engine_Api::_()->getDbtable('themes', 'sespwa')->fetchAll();
        ...
        $template_dir    = APPLICATION_PATH . '/application/themes/sespwa/'.$row->name;
    ...
    public function cloneAction()
    {
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            $orig_dir = APPLICATION_PATH . '/application/themes/sespwa/'.$orig_theme;
            ...
            try {
                Engine_Api::_()->getDbtable('themes', 'sespwa')->createRow(array(
                    'name'  => $new_theme['name'],
                    'title' => $new_theme['title'],
                    'description' => $new_theme['description'],
                ));
                Engine_Api::_()->getDbtable('themes', 'sespwa')->insert(array(
                    'name'  => $new_theme['name'],
                    'title' => $new_theme['title'],
                    'description' => $new_theme['description'],
                ));
    ...
    public function deleteAction()
    {
        ...
        $dir = APPLICATION_PATH . '/application/themes/sespwa/' . $this->_getParam('name');
        ...
        try {
            ...
            Engine_Api::_()->getDbtable('themes', 'sespwa')->delete(array(
                'name = ?' => $this->view->name,
            ));
    ...
    public function createColorVariantAction()
    {
        ...
        $masterDirectory = APPLICATION_PATH . '/application/themes/sespwa/' . $selectedTheme;
